...
{
    /// <summary>
    /// Converts a model of version 3 to a model of the current version
    /// </summary>
    public class ModelConverter
    {

        public RootModel ConvertToCurrentVersion(Model.V3.Root.RootModel root)
        {
        ...
        private void ConvertDataModel(DataModel newData, V3.Data.DataModel oldData)
        {
        ...
        //Copying Variables
        private VariablesModel ConvertVariablesModel(V3.Variables.VariablesModel variables)
        {
            VariablesModel result = new VariablesModel();
            result.GroupNames = variables.GroupNames;

            foreach (V3.Variables.VariableModel variable in variables.VariablesList)
            {
        ...
        private VariableModel ConvertVariableModel(V3.Variables.VariableModel variable)
        {
        ...
        private SequenceGroupModel ConvertSequenceGroup(DataModel parent, V3.Data.SequenceGroups.SequenceGroupModel group)
        {
            //Discard group name
            SequenceGroupModel result = new SequenceGroupModel(parent);

            foreach (V3.Data.Cards.CardBasicModel card in group.Cards)
            {
        ...
        private CardBasicModel ConvertCard(SequenceGroupModel parent, V3.Data.Cards.CardBasicModel card)
        {
            ...
            foreach (V3.Data.Channels.ChannelSettingsModel settings in card.Settings)
            {
                result.Settings.Add(ConvertChannelSettings(result, channel, settings));
                channel++;
            }

            foreach (V3.Data.Sequences.SequenceModel sequence in card.Sequences)
            {
        ...
        private ChannelSettingsModel ConvertChannelSettings(CardBasicModel parent, int channel, V3.Data.Channels.ChannelSettingsModel channelSettings)
        {
        ...
        private SequenceModel ConvertSequence(CardBasicModel parent, V3.Data.Sequences.SequenceModel sequence)
        {
            SequenceModel result = new SequenceModel(parent);
            result.Name = sequence.Name;
            result.startTime = sequence.startTime;

            foreach (V3.Data.Channels.ChannelBasicModel channel in sequence.Channels)
            {
        ...
        private ChannelBasicModel ConvertChannel(SequenceModel parent, V3.Data.Channels.ChannelBasicModel channel)
        {
            ChannelBasicModel result = new ChannelModel(parent);

            foreach (V3.Data.Steps.StepBasicModel step in channel.Steps)
            {
        ...
        private StepBasicModel ConvertStep(ChannelBasicModel parent, V3.Data.Steps.StepBasicModel step)
        {
            StepBasicModel result = null;

            if (step is V3.Data.Steps.StepFileModel)
            {
                V3.Data.Steps.StepFileModel fileStep = (V3.Data.Steps.StepFileModel)step;
            ...
            {
                V3.Data.Steps.StepRampModel rampStep = (V3.Data.Steps.StepRampModel)step;
        ...
        private ValueDoubleModel ConvertValueDoubleModel(V3.BaseTypes.ValueDoubleModel valueModel)
        {
