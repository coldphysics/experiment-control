...
{
    /// <summary>
    /// Converts a model of version 4 to a model of the current version
    /// </summary>
    public class ModelConverter
    {

        public RootModel ConvertToCurrentVersion(Model.V4.Root.RootModel root)
        {
        ...
        private void ConvertDataModel(DataModel newData, V4.Data.DataModel oldData)
        {
            newData.variablesModel = ConvertVariablesModel(oldData.variablesModel);
            newData.group = ConvertSequenceGroup(newData, oldData.group);
        }
        #endregion

        #region Converting Variables
        //Copying Variables
        private VariablesModel ConvertVariablesModel(V4.Variables.VariablesModel variables)
        {
            VariablesModel result = new VariablesModel();
            result.GroupNames = variables.GroupNames;

            foreach (V4.Variables.VariableModel variable in variables.VariablesList)
            {
        ...
        private VariableModel ConvertVariableModel(V4.Variables.VariableModel variable)
        {
        ...
        private SequenceGroupModel ConvertSequenceGroup(DataModel parent, V4.Data.SequenceGroups.SequenceGroupModel group)
        {
            //Discard group name
            SequenceGroupModel result = new SequenceGroupModel(parent);

            foreach (V4.Data.Cards.CardBasicModel card in group.Cards)
            {
        ...
        private CardBasicModel ConvertCard(SequenceGroupModel parent, V4.Data.Cards.CardBasicModel card)
        {
            ...
            foreach (V4.Data.Channels.ChannelSettingsModel settings in card.Settings)
            {
                result.Settings.Add(ConvertChannelSettings(result, channel, settings));
                channel++;
            }

            foreach (V4.Data.Sequences.SequenceModel sequence in card.Sequences)
            {
        ...
        private ChannelSettingsModel ConvertChannelSettings(CardBasicModel parent, int channel, V4.Data.Channels.ChannelSettingsModel channelSettings)
        {
        ...
        private SequenceModel ConvertSequence(CardBasicModel parent, V4.Data.Sequences.SequenceModel sequence)
        {
            SequenceModel result = new SequenceModel(parent);
            result.Name = sequence.Name;
            result.startTime = sequence.startTime;

            foreach (V4.Data.Channels.ChannelBasicModel channel in sequence.Channels)
            {
        ...
        private ChannelBasicModel ConvertChannel(SequenceModel parent, V4.Data.Channels.ChannelBasicModel channel)
        {
            ChannelBasicModel result = new ChannelModel(parent);

            foreach (V4.Data.Steps.StepBasicModel step in channel.Steps)
            {
        ...
        private StepBasicModel ConvertStep(ChannelBasicModel parent, V4.Data.Steps.StepBasicModel step)
        {
            StepBasicModel result = null;

            if (step is V4.Data.Steps.StepFileModel)
            {
                V4.Data.Steps.StepFileModel fileStep = (V4.Data.Steps.StepFileModel)step;

                result = new StepFileModel(parent, (StepFileModel.StoreType)fileStep.Store);
                (result as StepFileModel).FileName = fileStep.FileName;
            }
            else if(step is V4.Data.Steps.StepRampModel)
            {
                V4.Data.Steps.StepRampModel rampStep = (V4.Data.Steps.StepRampModel)step;

                result = new StepRampModel(parent, (StepRampModel.StoreType)rampStep.Store);
            } else
            {
                V4.Data.Steps.StepPythonModel pyStep = (V4.Data.Steps.StepPythonModel)step;

                result = new StepPythonModel(parent);
                (result as StepPythonModel).Script = pyStep.Script;
            }
        ...
        private ValueDoubleModel ConvertValueDoubleModel(V4.BaseTypes.ValueDoubleModel valueModel)
        {
